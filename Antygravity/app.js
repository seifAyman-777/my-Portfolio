...
const specCopyDatabase = {
    ...
    'sales-page-spec': {
        title: "Sales Page Copywriting Blueprint",
        badge: "Core Deliverable Spec",
        subtitle: "Architecture for Long-Form Landing Pages & High-Ticket Offers",
        content: `
            <div class="spec-detail-box">
                <p>Long-form sales page copy built to systematically eliminate buyer hesitation. Includes hero positioning, pain agitation, value stack breakdown, social proof placement, risk-reversal guarantees, and frictionless CTA anchors.</p>
            </div>
        `
    },
    'email-sequence-spec': {
        title: "Email Sequence & Marketing Blueprint",
        badge: "Core Deliverable Spec",
        subtitle: "High-Open Automated Sequences & Retention Campaigns",
        content: `
            <div class="spec-detail-box">
                <p>Open-loop email sequences designed to build relationship momentum and generate predictable recurring sales. Covers Welcome Flows, Cart Abandonment, and Weekly Value Newsletters.</p>
            </div>
        `
    },
    'ad-copy-spec': {
        title: "Ad Copy & Social Hooks Blueprint",
        badge: "Core Deliverable Spec",
        subtitle: "Scroll-Stopping Meta & Google Ad Creative Angles",
        content: `
            <div class="spec-detail-box">
                <p>Meta and Google ad creative hooks paired with primary text, headlines, and callout angles engineered to lower Cost-Per-Acquisition (CPA) and boost click-through rates.</p>
            </div>
        `
    },
    'headlines-spec': {
        title: "Headlines & Micro-Copy Blueprint",
        badge: "Core Deliverable Spec",
        subtitle: "Instant Value Proposition Refinement",
        content: `
            <div class="spec-detail-box">
                <p>Website headline polishing, subhead framing, form label micro-copy, and announcement hooks designed to stop bounce rates and elevate page clarity.</p>
            </div>
        `
    }
};
...
async function handleFormSubmit(event) {
    ...
    if (btnText) btnText.textContent = "Sending to Supabase...";
    ...
    try {
        if (supabaseClient) {
            // Primary insert into 'contacts' table
            let { data, error } = await supabaseClient.from('contacts').insert([formData]);
            
            if (error) {
                console.warn("Notice: Table 'contacts' insert error:", error.message);
                // Secondary fallback attempt into 'leads' table
    ...
    console.log("⚡ [SUPABASE SUBMISSION STATUS]:", supabaseSaved ? "SUCCESS" : "LOCAL FALLBACK", formData);
    
    // Save locally for debug/verification
    window.latestSubmission = formData;
    formData.supabaseSaved = supabaseSaved;
    ...
    // Trigger Auto-Confirmation Modal
...
function showConfirmationModal(data) {
    ...
    document.getElementById('confClientName').textContent = data.brand_name;
    document.getElementById('confBrandName').textContent = data.brand_name;
    document.getElementById('confEmailTo').textContent = data.email;
    document.getElementById('confFirstName').textContent = data.brand_name;
    document.getElementById('confBrandText').textContent = data.brand_name;
    document.getElementById('confPhoneText').textContent = data.whatsapp_number;

    // Update note badge based on database sync status
    const confNote = modal.querySelector('.conf-note');
    if (confNote) {
        if (data.supabaseSaved) {
            confNote.innerHTML = "✅ <strong>Successfully recorded in your Supabase Database (Table: contacts).</strong>";
            confNote.style.color = "#4ADE80";
        } else {
            confNote.innerHTML = "⚡ <strong>Submission received!</strong> Note: Make sure to create the <code>contacts</code> table in your Supabase SQL Editor if you haven't yet.";
            confNote.style.color = "#93C5FD";
        }
    }
